...
export default function HUD() {
  ...
  return (
    <>
      {/* Top Right - Secondary Navigation Stack (Slim Glass Design) */}
      <div className="fixed top-6 right-6 z-[100]">
        <motion.div 
          initial={{ x: 20, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ type: "spring", ...springConfig }}
          className="flex flex-col gap-3 p-1.5 bg-white/5 backdrop-blur-[10px] border border-white/20 rounded-full shadow-2xl"
        >
      ...
      {/* Bottom Center - Main Dock (Glassmorphism) */}
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100]">
        <motion.div 
          initial={{ y: 50, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          transition={{ type: "spring", ...springConfig }}
          className="flex items-center gap-1.5 p-1.5 bg-white/5 backdrop-blur-[12px] border border-white/20 rounded-2xl shadow-2xl"
        >
    ...
    <Link href={href}>
      <motion.div
        ...
        className={cn(
          ...
          active ? "bg-white/15 text-white shadow-[0_0_20px_rgba(255,255,255,0.1)]" : "text-white/40 hover:text-white"
        )}
      >
